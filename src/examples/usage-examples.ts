...
export class UseCaseExamples {
  private app: SupplierManagementApp;

  constructor() {
    this.app = new SupplierManagementApp();
  }
  ...
  async onboardNewSupplier(name: string, domain: string, contactName: string, contactEmail: string, contactTitle: string) {
  ...
  async prepareForMeeting(supplierName: string) {
